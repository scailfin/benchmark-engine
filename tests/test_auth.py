...
class TestUserAuthentication(object):
    ...
    def test_authenticate(self, tmpdir):
        ...
        # Login user 1 and 2
        user_1 = users.login(USER_1, USER_1)
        user_2 = users.login(USER_2, USER_2)
        # Authenticate user 1
        assert auth.authenticate(con, user_1.api_key).identifier == USER_1
        # Authenticate user 2
        assert auth.authenticate(con, user_2.api_key).identifier == USER_2
        # Logout user 1
        loggedout_user = users.logout(user_1.api_key)
        ...
        with pytest.raises(err.UnauthenticatedAccessError):
            auth.authenticate(con, user_1.api_key)
        # Logging out a user that is not logged in will raise error
        with pytest.raises(err.UnauthenticatedAccessError):
            users.logout(user_1.api_key)
    ...
    def test_login_timeout(self, tmpdir):
        ...
        api_key = users.login(USER_1, USER_1).api_key
        # Wait for two seconds
        time.sleep(2)
        # Authenticating after timeout will raise error
        with pytest.raises(err.UnauthenticatedAccessError):
            auth.authenticate(con, api_key)
